...
function M.Subtract(img2)
   return function(input)
        local perm = torch.LongTensor{3, 2, 1}
        input = input:index(1, perm):mul(256.0)
        input:add(-1, img2)
        return input
   end
end
...
function M.Jitter(ratio)
    return function(input)
        local rands = torch.rand(unpack(input:size():totable()))
        rands:mul(2):add(-1):mul(ratio):mul(input:max())
        input:add(rands)
        return input
    end
end
